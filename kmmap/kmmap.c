						...
						struct vm_fault *vmf)
{
	...
	page = vmalloc_to_page(pages + offset);
#endif
	if(!page)
...
static int my_module_init(void)
{
	...
	// keep from swap out
	viraddr = (unsigned long)pages;
	while(viraddr <= ((unsigned long)pages+PAGE_SIZE))
	{
		SetPageReserved(vmalloc_to_page((const void *)viraddr));
		viraddr += PAGE_SIZE;
	}

	strcpy(pages, "hello abcd");
	strcpy(pages+PAGE_SIZE, "hello defg");
	return ret;
}
